...
export default class OEmbed {
  ...
  static get sanitize() {
    return {
      ...
      html: true // only tags from Inline Toolbar
    };
  }
  ...
  set data(data) {
    ...
    const { service, source, embed, width, height, html, caption = "" } = data;
  ...
  get data() {
    if (this.element) {
      ...
      this._data.caption = caption ? caption.innerHTML : this._data.caption;
    }
  ...
  static prepare({ config = {} }) {
    ...
    const userServices = Object.entries(services)
      ...
      .filter(([key, service]) => OEmbed.checkServiceConfig(service))
      .map(([key, service]) => {
        const {
          regex,
          embedUrl,
    ...
    OEmbed.services = entries.reduce((result, [key, service]) => {
      if (!(key in result)) {
        result[key] = service;
        return result;
      }
  ...
  static checkServiceConfig(config) {
    const { regex, embedUrl, html, height, width, id, oembedUrl } = config;
    ...
    isValid = isValidRegex && (isValidEmbedUrl || isValidOEmbedUrl);
